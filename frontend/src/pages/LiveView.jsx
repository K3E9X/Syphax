...
export default function LiveView() {
  ...
  // Console is permanent (left rail), so it is not a tab. Overview replaces
  // what the old console tab carried below the stream: surface and cost.
  const TABS = [
  ...
  const statusText = active ? (run0.status === 'queued' ? 'queued' : 'running')
    : (run0.status || 'not started');
    ...
    <div className="page lv2">
      ...
      <header className="lv2-head">
        <div className="lv2-head__id">
          <button className="btn btn--muted btn--sm" onClick={() => nav('/engagements')}>← Back</button>
          <h1 className="lv2-head__h1">Live view</h1>
          {host && <span className="lv2-head__host mono">{host}</span>}
          <span className={'lv2-status lv2-status--' + (active ? 'run' : (run0.stop_reason ? 'stopped' : 'idle'))}>
            {active && <span className="live-dot pulse" />}
            {statusText}
          </span>
        </div>
        <div className="lv2-head__actions">
          {!active && <button className="btn btn--solid" onClick={run} disabled={busy}>Run</button>}
          {active && <button className="btn btn--danger" onClick={stop} disabled={busy}>Stop</button>}
          <button className="btn" onClick={analyze} disabled={busy} title="Mine everything captured through the proxy">Deep analysis</button>
          ...
          <a className="btn btn--muted" href={`/api/engagements/${id}/report.html`} target="_blank" rel="noreferrer">Print</a>
        </div>
      </header>

      {/* ---- phase rail: where the run is, as a connected track ---- */}
      <div className="lv2-rail" role="list" aria-label="Run phases">
        {PHASES.map((ph, i) => {
          const st = phaseIdx < 0 ? 'pending' : i < phaseIdx ? 'done' : i === phaseIdx ? 'active' : 'pending';
          return (
            <div key={ph} className={'lv2-phase lv2-phase--' + st} role="listitem">
              <span className="lv2-phase__dot" />
              <span className="lv2-phase__name">{PHASE_LABEL[ph]}</span>
            </div>
          );
        })}
      </div>
      <div className="lv2-meta">
        {run0.stale && <span className="run-reason">no worker reporting — Run reclaims it</span>}
        {!active && run0.stop_reason && <span className="run-reason">{REASON_LABEL[run0.stop_reason] || run0.stop_reason}</span>}
        <span>iteration <b>{run0.iterations || 0}</b></span>
        <span className="sep">·</span><span><b>{run0.jobs_launched || 0}</b> jobs</span>
      </div>

      {/* ---- KPIs: the numbers that matter, always in view ---- */}
      <div className="lv2-kpis">
        <div className="stat stat--alert"><div className="stat__l">Confirmed</div><div className="stat__v">{vsum.confirmed || 0}</div></div>
        <div className="stat"><div className="stat__l">Findings</div><div className="stat__v">{findingsAll.length}</div></div>
        <div className="stat"><div className="stat__l">FP rate</div><div className="stat__v">{vsum.false_positive_rate_pct ?? 0}<small>%</small></div></div>
        <div className="stat"><div className="stat__l">Assets</div><div className="stat__v">{assets.length}</div></div>
        <div className="stat"><div className="stat__l">Jobs</div><div className="stat__v">{run0.jobs_launched || 0}</div></div>
        ...
        <div className="stat"><div className="stat__l">Cost</div><div className="stat__v"><small>$</small>{(llm.cost_usd || 0).toFixed(4)}</div></div>
      </div>
      ...
      {/* Approvals block the run: an unmissable banner, never behind a tab. */}
      {approvals.map((a) => (
        <div key={a.id} className="approval approval--banner">
          <div className="approval__t">⏸ Approval required to continue</div>
          <div className="approval__d">{a.summary}</div>
          <div className="approval__meta">tools: {(a.tools || []).join(', ')} · targets: {(a.targets || []).join(', ')}</div>
          <div className="approval__row">
            <button className="btn btn--solid" onClick={() => decide(a.id, 'approved')}>Approve exploitation</button>
            <button className="btn btn--danger" onClick={() => decide(a.id, 'denied')}>Deny</button>
          </div>
        </div>
      ))}

      {/* ---- split body: live console left, drill-down right ---- */}
      <div className="lv2-body">
        <section className="lv2-console-col">
          <div className="card lv2-console-card">
            <div className="card__head">
              <span className="card__title">Agent console</span>
              <span className="card__meta">
                {active && <span className="live-dot pulse" />} {info.length} events
              </span>
            </div>
            <div className="console lv2-console" ref={agentRef}>
          ...
          <details className="lv2-verbose" open={verbose.length > 0 && verbose.length < 40}>
            <summary>Verbose — raw tool I/O ({verbose.length})</summary>
        ...
        <section className="lv2-detail-col">
          <div className="tabs lv2-tabs">
            {TABS.map((t) => (
          ...
          {tab === 'overview' && (
            ...
            <div className="card">
              ...
              <div className="card__body">
                {tech.length === 0 && assets.length === 0
                  ? <div className="empty">Nothing discovered yet. The run populates this as it maps the target.</div>
                  : (
                    <>
                      {tech.length > 0 && (
                        <div className="techrow">
                          <span className="stat__l" style={{ margin: 0 }}>Technologies</span>
                          {tech.map((t) => <span key={t} className="tech-chip">{t}</span>)}
                        </div>
                      )}
                      <p className="home-intro" style={{ marginTop: 12 }}>
                        {assets.length} asset(s) discovered · {coverage.length} catalog test(s) run ·{' '}
                        {chains.length} kill-chain(s). Open a tab to drill in.
                      </p>
                    </>
                  )}
              </div>
                  ...
                  <tbody>
                    {assets.map((a, i) => (
                      <tr key={i}>
                        ...
                        <td className="mono truncate" style={{ maxWidth: 300 }} title={a.value}>{a.value}</td>
          ...
          {tab === 'jobs' && (
            <div className="card">
              <div className="card__head"><span className="card__title">Scan jobs</span><span className="card__meta">click a row for the command &amp; output</span></div>
              <div className="tbl-scroll">
                <table className="tbl">
          ...
          {tab === 'findings' && (
            <div className="card">
              <div className="card__head"><span className="card__title">Validated findings</span><span className="card__meta">{findingsAll.length} total · {vsum.confirmed || 0} confirmed</span></div>
              <div className="card__body">
                ...
                {findings.map((f) => (
                  <div key={f.id} className={'finding finding--' + (f.severity || 'info')}>
                    <div className="finding__top">
                      <span className={'sev sev--' + (f.severity || 'info')}>{f.severity}</span>
                      <span className={'vstatus vstatus--' + f.status}>{f.status} · {Math.round((f.confidence || 0) * 100)}%</span>
                      <span className={'proof proof--' + (f.proven ? 'yes' : 'no')}
                            title={f.proven ? 'Reproduced against the target by an oracle'
          ...
          {tab === 'tokens' && (
            <TokenPanel usage={usage.data} loading={usage.loading} error={usage.error} onRetry={usage.reload} />
          )}
        </section>
      </div>
