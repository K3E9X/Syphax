...
async function mount() {
  let r;
  await act(async () => { r = render(<MemoryRouter><Recon /></MemoryRouter>); });
  return r;
}

async function search(term = 'example.com') {
  await userEvent.type(screen.getByLabelText('Target'), term);
  await userEvent.click(screen.getByRole('button', { name: /run recon/i }));
}

beforeEach(() => {
  vi.restoreAllMocks();
  mockNavigate.mockClear();
  vi.spyOn(api.recon, 'scope').mockResolvedValue(SCOPE);
});
...
describe('what it tells the operator it will do', () => {
  it("lists the backend's scope, not a copy of it", async () => {
    ...
    expect(api.recon.scope).toHaveBeenCalled();
  });

  it('says active testing needs an engagement before anything has been run', async () => {
    await mount();
    expect(screen.getByText(/active testing needs an engagement/i)).toBeTruthy();
  });
});

describe('running it', () => {
  it('shows the rejection sentence the backend wrote', async () => {
    vi.spyOn(api.recon, 'run').mockRejectedValue(
  ...
  it('passes the CT toggle through, because it is the slow source', async () => {
    const run = vi.spyOn(api.recon, 'run').mockResolvedValue(REPORT);
    await mount();
  ...
  it('puts the synthesis above the raw panels', async () => {
    vi.spyOn(api.recon, 'run').mockResolvedValue(REPORT);
    await mount();
    await search();
  ...
  it('keeps the three technology layers apart', async () => {
    ...
    const column = (name) => screen.getByText(name).closest('.rec-tech__col');
  ...
  it('surfaces a source that did not answer instead of pretending it had nothing', async () => {
    vi.spyOn(api.recon, 'run').mockResolvedValue({
      ...REPORT,
